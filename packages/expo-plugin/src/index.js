...
function withGlanceKit(config, props = {}) {
  ...
  config = withAndroidCoreSettings(config);
...
function withAndroidCoreSettings(config) {
  return withSettingsGradle(config, modConfig => {
    modConfig.modResults.contents = ensureAndroidCoreProject(
...
function ensureAndroidCoreProject(contents) {
  if (contents.includes("include ':glancekit-android-core'")) {
    return contents;
  }
...
def glanceKitAndroidCoreDir = new File(
  ["node", "--print", "require.resolve('@glancekit/android-core')"]
